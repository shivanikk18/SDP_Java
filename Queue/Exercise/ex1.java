...
class Tester {
    ...
    while (!queue.isEmpty()) {
        ...
        if (val % 2 == 0) {
            evenQueue.enqueue(val); // Enqueue even numbers
        } else {
            oddQueue.enqueue(val); // Enqueue odd numbers
        }
    }

    Queue[] queueArray = new Queue[2];
    queueArray[0] = oddQueue; // 0th index contains odd numbers queue
    queueArray[1] = evenQueue; // 1st index contains even numbers queue
